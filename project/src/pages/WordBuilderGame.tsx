...
const WordBuilderGame: React.FC<WordBuilderGameProps> = ({ language, grade, showToast }) => {
  ...
  const handleTryAgain = () => {
    // Reset the current question state to allow user to try again
    const shuffledTiles = [...currentItem.tiles].sort(() => Math.random() - 0.5);
    setAvailableTiles(shuffledTiles);
    setAnswerSlots([]);
    setShowFeedback(false);
    setIsCorrect(false);
    setShowHint(false);
    // Don't reset attempts - keep track of total attempts
  };
        ...
        <div className="flex justify-center space-x-4 mb-6">
          {!showFeedback ? (
            <>
              ...
              <button
                onClick={handleReset}
                className="bg-gray-500 hover:bg-gray-600 text-white px-6 py-2 rounded-xl font-medium transition-colors flex items-center space-x-2"
              >
                <RefreshCw className="w-4 h-4" />
                <span>Reset</span>
              </button>
